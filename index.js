...
function diemDanh() {
  const userId = "<?php echo $_SESSION['id']; ?>"; // Lấy userId từ PHP
  const today = new Date();
  const lastCheckIn = localStorage.getItem(`lastCheckIn_${userId}`);

  // Lấy số ngày tích lũy từ localStorage hoặc khởi tạo bằng 0
  let accumulatedDays =
    Number(localStorage.getItem(`accumulatedDays_${userId}`)) || 0;
  let dayStreak = Number(localStorage.getItem(`dayStreak_${userId}`)) || 0;
  let chuoiNgay = JSON.parse(localStorage.getItem(`chuoiNgay_${userId}`)) || [];
  ...
  localStorage.setItem(`chuoiNgay_${userId}`, JSON.stringify(chuoiNgay));

  // Lưu số ngày tích lũy và liên tiếp vào localStorage
  localStorage.setItem(`accumulatedDays_${userId}`, accumulatedDays);
  localStorage.setItem(`dayStreak_${userId}`, dayStreak);
...
function checkLastCheckIn() {
  const userId = "<?php echo $_SESSION['id']; ?>"; // Lấy userId từ PHP
  const chuoiNgay =
    JSON.parse(localStorage.getItem(`chuoiNgay_${userId}`)) || [];
  const dayStreak = Number(localStorage.getItem(`dayStreak_${userId}`)) || 0;
